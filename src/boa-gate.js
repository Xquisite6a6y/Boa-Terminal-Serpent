...
function sealPayload(secret, payload) {
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv('aes-256-gcm', encryptionKey(secret), iv);
  const encrypted = Buffer.concat([cipher.update(payloadToString(payload), 'utf8'), cipher.final()]);
  return {
...
function openPayload(secret, envelope) {
  if (!envelope.iv || !envelope.tag || envelope.bodyEncoding !== 'aes-256-gcm-base64url') {
    return Buffer.from(envelope.body, 'base64url').toString('utf8');
  }
...
function wrapOutbound(payload, context = {}, gate = activeGate) {
  ...
  const sealed = sealPayload(config.transportSecret, payload);
  const body = sealed.cipherText;
  const envelope = {
    ...
    body,
    bodyEncoding: 'aes-256-gcm-base64url',
    iv: sealed.iv,
    tag: sealed.tag,
    trustGateway: 'authorized-channel',
  };
...
function unwrapInbound(payload, context = {}, gate = activeGate) {
  ...
  let unwrapped;
  try {
    unwrapped = openPayload(config.transportSecret, payload);
  } catch (error) {
    recordEvent(gate, 'Signal failed authentication', { envelopeId: payload.id, preview: '[opaque BOA envelope]' });
    return { ok: false, code: 'BOA_GATE_REJECTED_UNTRUSTED_ENVELOPE' };
  }
...
function gateStatus(gate = activeGate) {
  ...
  return {
    ...
    operatingModel: 'daemon-first translator trust gateway',
    authorizedScope: 'BOA-routed channels; OS VPN/firewall integrations required for full-device interception',
